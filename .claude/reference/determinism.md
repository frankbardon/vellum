...
| xlsx `styles.xml` | Fixed preamble with builtin `numFmts`/`fonts`/`fills`/`borders` indices intact. Fill index 0 and 1 are reserved by the spec; getting it wrong makes Excel refuse to open the file. Pinned by unit test (`TestStylesXML_ReservedFillIndicesArePresentEvenWithNoCustomFills`) and golden-corpus determinism (`xlsx-compose`). |
| xlsx cell comments | A comment is two parts kept in agreement: `xl/comments1.xml` states the text and `xl/drawings/vmlDrawing1.vml` states the legacy VML shape that draws it, one shape per comment, matched by position. The worksheet's `<legacyDrawing>` reference and both parts' relationships are built together so none of the three can name a comment the others do not. |
| xlsx table annotation column | `styleBuilder` and the annotation-doubling decision are made once per table from a single pre-scan (does any cell carry an annotation), not per cell — so the same table's columns cannot be doubled on one row and single on another depending on iteration order. |
...
| Line breaking | Integer arithmetic from the font's own design units throughout. Never accumulate widths in `float64`. The conversion to text space happens once per styled run per line — never per glyph, which rounds sixty times down a line and accumulates; and it cannot be once per line, because a styled run brings its own size and units per em and there is no single denominator. |
| Pagination | Decided by `pdf.Lower`, greedily, and part of the bytes. There is no application behind a PDF to lay text out afterwards, so a page count is Vellum's answer rather than a reader's. A paragraph's leading is derived once from its tallest line, not per line, so a paragraph sets at one rhythm. |
| Footnote placement | The space a note will occupy raises the page's floor before the body that follows it is placed. After the fact the body is already sitting where the note has to go, and the two are drawn on top of each other — which renders, and is wrong. `TestLower_FootnotesRaiseTheFloorForTheBody` fails within one page of removing the reserve. A note whose mark will not fit above its own body moves to the next page with its mark: a mark on one page and its text on another is the one thing a footnote may not do. |
| PDF table overflow | Page capacity is **measured**, not theme-derived, and that is the one place the two rules differ. Vellum does not lay out OOXML, so a pptx capacity measured from content would carry the measuring machine's fonts into the split; it lays PDF out completely, so every row height here is a number it computed from the theme's own faces and then drew. The greedy fill is shared — `overflow.PlanRows` holds it and `overflow.PlanTable` delegates to it — so a boundary cannot fall in two places. Rows are measured before any split is planned, and each stub position is measured carrying its own label even where a merge above covers it, because `fragment.ClipStub` restarts a merge on a continuation page and a height that changed with the plan would be a capacity for a table nobody draws. |
| PDF table paint order | Every cell's fill, then every cell's hairline, then every cell's text — three passes over the same rows, not one. PDF paints in the order the operators appear, so a fill emitted after a neighbour's hairline covers the hairline and text emitted before a fill vanishes under it. Three passes is the only order in which each cell can be emitted independently and still land right. `TestTable_FillsAndRulesPrecedeText` reads the order off a page carrying nothing but the table. |
| PDF page resources | Derived from the page's items rather than listed beside them, in the order the page draws. A caller cannot name a font the stream does not select, or select one the dictionary does not name. |
| Font subsetting order | Every page's content stream is built before any font object is written, because a subset is the glyphs the document actually drew. `TestGoldensNotHandEdited` would catch a reordering; the font writer refusing an empty subset catches it first. |
...
| Decode bound | `image.Options.MaxDecodedBytes`, default 256 MiB, checked against the header **by division rather than by multiplication**. A PNG declares its size in eight bytes and nothing obliges them to be true: a 2^31-1 square at sixteen bits across four channels describes about 2^70 bytes, so computing the product first overflows int64 to a negative number, the comparison passes, and `make` panics. `FuzzImage` found that in seven seconds; `TestPNG_HugeDimensionsAreRefusedWithoutAllocating` pins it. |
