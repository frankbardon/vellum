...
| xlsx `styles.xml` | Fixed preamble with builtin `numFmts`/`fonts`/`fills`/`borders` indices intact. Fill index 0 and 1 are reserved by the spec; getting it wrong makes Excel refuse to open the file. Pinned by unit test (`TestStylesXML_ReservedFillIndicesArePresentEvenWithNoCustomFills`); golden corpus lands with E6-S5. |
