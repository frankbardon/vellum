...
func TestNoUnsortedMapIteration(t *testing.T) {
	...
	walkSource(t, func(path string, file *ast.File, fset *token.FileSet) {
		...
		ast.Inspect(file, func(n ast.Node) bool {
			...
			if !ok || !maps[ident.Name] {
				return true
			}
			// Ranging for keys alone, to sort them, is the sanctioned shape and
			// is what the accumulate-then-sort idiom looks like. Ranging for
			// values is the one that puts map order into output.
...
// mapValuedNames collects the identifiers in a file whose declared type is
// syntactically a map.
//
...
//
// A map reached through a call or through a selector is not seen, so this gate
// is a floor rather than a proof.
//
// Names are collected per file rather than per scope, so a struct field called
// `themes` makes every local called `themes` look like a map — including a
// parameter that is a slice. The false positive is loud and is fixed by
// renaming, which costs a line; teaching the gate about scopes would cost a
// resolver, and a gate nobody can read is a gate people learn to suppress.
func mapValuedNames(file *ast.File) map[string]bool {
	out := make(map[string]bool)

	record := func(names []*ast.Ident, typ ast.Expr, values []ast.Expr) {
		isMap := false
	...
	ast.Inspect(file, func(n ast.Node) bool {
		switch d := n.(type) {
		case *ast.ValueSpec:
			record(d.Names, d.Type, d.Values)
		case *ast.AssignStmt:
