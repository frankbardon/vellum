...
func (w *writer) corePropsXML() []byte {
	...
	b.WriteString(`<cp:coreProperties xmlns:cp="` + nsCoreProps +
		...
		`" xmlns:xsi="` + nsXSI + `">`)
	if w.deck.Title != "" {
		b.WriteString(`<dc:title>` + escapeText(w.deck.Title) + `</dc:title>`)
	}
...
// are the ones a reader can actually see: PowerPoint shows them in File > Info >
...
func (w *writer) customPropsXML() []byte {
	...
	for i, p := range props {
		...
		// and a package numbering from 0 is one PowerPoint refuses to open.
...
// scalar and a reader looking at the properties dialogue should see fields
...
// auditable without becoming the package's largest part.
func provenanceProperties(w *writer) []customProperty {
	...
	if r.GeneratedAt != nil {
		...
		// output. Its absence is therefore itself information: a deck with
